...
request('POST /workspaces/:workspaceSlug/notes/:noteId/unarchive', () => {
  response.status('200', () => {
    context('when user has access to workspace', () => {
      it('archives the note', async () => {
        ...
        const response = await route.request('/api/workspaces/wp-1/notes/note-1/unarchive', {
          method: 'POST',
        ...
        expect(updatedNote).toMatchObject({ archivedAt: null });
      });
    });
  });
